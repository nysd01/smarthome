...
from .models import Room, Device, Temperature
...
def dashboard(request):
    ...
    fire_alert = temperature > 30

    # ✅ Get 24h data for chart
    last_24h = now() - timedelta(hours=24)
    temps = Temperature.objects.filter(timestamp__gte=last_24h).order_by('timestamp')
    temp_chart_data = {
        "labels": [t.timestamp.strftime("%H:%M") for t in temps],
    ...
    return render(request, 'control/dashboard.html', {
        ...
        'temp_chart_data': json.dumps(temp_chart_data),
    })
...
def toggle_device(request, device_id):
    ...
    topic = f"nysd/derek/led/{device.id}/control"
